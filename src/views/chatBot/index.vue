<template>
        ...
        <div class="content">
          <div class="itemInfo" v-for="item in contentList" :key="item.id">
            <div class="topBox">
              ...
              <el-button size="mini" round @click="changeBot(item)">使用</el-button>
 ...
 export default {
   ...
   methods: {
      changeBot(item) {
       console.log('changeBot', item)
       this.Util.setLocal('robotInfo', item)
     },
     cancel(){
     ...
     getGroupList() {
       this.axios.listGroup().then(data => {
         ...
         // data.data.data增加一个全部
         data.data.data.unshift({ id: 'all', name: '全部',  })
     ...
     handleCategoryDelete(val){
       this.$confirm('删除后，该分类下的智能体将自动移动到全部是否确认删除', '确认删除该分类？', {
